...
# Add affected_local_code
# Existing date grid with years and months
years <- c(2020, 2021, 2022, 2023)
months <- 1:12
date_grid <- expand.grid(year = years, month = months)
...
local_policy_summary <- date_grid
  ...
  for (j in 1:nrow(date_grid)) {
    ...
    if (!(end_date < year_month_start || start_date > year_month_end)) {
      ...
      if (length(summary_index) == 1) {
        local_policy_summary[summary_index, "policy_type"] <- local_policy_summary[summary_index, "policy_type"] + policy_type
        local_policy_summary[summary_index, "policy_category"] <- local_policy_summary[summary_index, "policy_category"] + policy_category
      }
    }
  }
}
...
# Assuming local_policy, date_grid, and local_policy_summary are already defined and available

# Register the parallel backend with a specified number of cores
numCores <- detectCores()
registerDoParallel(cores = numCores)

# Parallel loop to process local_policy
local_policy_summary_parallel <- foreach(i = 1:nrow(local_policy), .combine = 'rbind') %dopar% {
  policy_row <- local_policy[i, ]
  
  # Initialize row_results
  ...
  # Relaxing == -1, Restricting == +1
  policy_type <- ifelse(policy_row$policy_type == "Relaxing", -1, +1)
  
  # Facemask == +1, Social distancing == -1
  policy_category <- ifelse(policy_row$policy_category == "Face mask", +1, -1)
  ...
  for (j in 1:nrow(date_grid)) {
    ...
    if (!(end_date < year_month_start || start_date > year_month_end)) {
      ...
      if (length(summary_index) == 1) {
        new_row <- local_policy_summary[summary_index, ]
        new_row$policy_type <- new_row$policy_type + policy_type
        new_row$policy_category <- new_row$policy_category + policy_category
        row_results <- rbind(row_results, new_row)
      }
    }
  }
...
final_results <- do.call(rbind, local_policy_summary_parallel)

# Stop the parallel backend
stopImplicitCluster()

# final_results now contains the accumulated results
